...
# ---------------------------------------------------------------------------
def solve_pipe(d1_mm: float, k: float, tf: float, ta: float, target: float,
    ...
    # 找上界
    for _ in range(60):
        Ts, q_flux, q_l = surface_state_pipe(d1_mm, k, tf, ta, upper, 10.0)  # 初估 h
    ...
    for _ in range(200):
        ...
        # 自洽迭代 h_c (Ts 依赖 h, h 依赖 Ts)
        Ts_guess = ta + 0.5 * (tf - ta) if is_heating else ta + 0.5 * (tf - ta)
        for _inner in range(30):
            hc = hc_cylinder_astm(d1_mm / 1000.0 + 2 * delta / 1000.0,
                                  Ts_guess, ta, v)
            hr = hr_radiation(epsilon, Ts_guess, ta)
            h = hc + hr
            Ts, q_flux, q_l = surface_state_pipe(d1_mm, k, tf, ta, delta, h)
    ...
    for _ in range(50):
        hc = hc_cylinder_astm(d1_mm / 1000.0 + 2 * delta / 1000.0, Ts_guess, ta, v)
        hr = hr_radiation(epsilon, Ts_guess, ta)
        h = hc + hr
        Ts, q_flux, q_l = surface_state_pipe(d1_mm, k, tf, ta, delta, h)
        if abs(Ts - Ts_guess) < 0.005:
            break
        Ts_guess = Ts
...
def solve_flat(k: float, tf: float, ta: float, target: float, mode: str,
               v: float, epsilon: float, length_m: float, width_m: float):
    is_heating = tf > ta
    lower, upper = 0.01, 1.0
    for _ in range(60):
        Ts, q_flux = surface_state_flat(k, tf, ta, upper, 10.0)
        if mode in ('surface', 'condensation'):
            if (is_heating and Ts < target) or ((not is_heating) and Ts > target):
                break
        else:
    ...
    for _ in range(200):
        ...
        for _inner in range(30):
            ...
            hr = hr_radiation(epsilon, Ts_guess, ta)
            h = hc + hr
            Ts, q_flux = surface_state_flat(k, tf, ta, delta, h)
            if abs(Ts - Ts_guess) < 0.01:
                break
            Ts_guess = 0.5 * Ts_guess + 0.5 * Ts
...
def calculate(case: dict) -> dict:
    ...
    if equip == 'pipe':
        r = solve_pipe(case['outerDiameter'], k, tf, ta, target, mode, v, eps)
        linear = r['linear_heat_loss_w_m']
        annual = linear * hours / 1000.0
    else:
        r = solve_flat(k, tf, ta, target, mode, v, eps,
