#!/usr/bin/env node
// Verification of corrected flame temperature algorithm
...
function enthalpy(species, T) {
  ...
  if (!a) {
    const cpMap = { 'C₂H₆': 0.053, 'C₅H₁₂': 0.12, 'C₆H₁₄': 0.14, 'C₇H₁₆': 0.16, 'C₆H₆': 0.082, 'C₂H₄': 0.043, 'C₃H₆': 0.059, 'C₄H₈': 0.072, 'C₂H₂': 0.044, 'NH₃': 0.036, 'H₂S': 0.034 };
...
// Corrected element potential method for chemical equilibrium at P = 1 bar
function equilibriumComposition(b, T) {
  ...
  // Compute pi_i = exp[(sum_j lambda_j*a_ij - mu_i)/RT]
  function computePi(lambda) {
    return activeSpecies.map((sp, i) => {
      const comp = atomicComp[sp];
  ...
  // Initial guess for lambda (same as frontend)
  ...
  // Use last element as reference for ratios
  ...
  for (let iter = 0; iter < 300; iter++) {
    ...
    // Element sums: sumEl[j] = sum_i a_ij * pi_i
    ...
    if (err < 1e-10) {
      // Compute n = b_j / sumEl[j] (should be same for all j)
    ...
    // Jacobian J[j][k] = dRvec[j]/dlambda[k]
    ...
    for (let k = 0; k < ne; k++) {
      ...
      // For ratio residuals
      for (let j = 0; j < ne - 1; j++) {
        // dRvec[j]/dlambda[k] = b_j * sum_i a_i,ne * a_ik * pi_i / RT - bRef * sum_i a_ij * a_ik * pi_i / RT
        let crossJ = 0;
        let crossRef = 0;
        for (let i = 0; i < ns; i++) {
    ...
    for (let attempt = 0; attempt < 15; attempt++) {
      ...
      const oldErr = err;
      if (newErr < oldErr || stepScale < 1e-6) {
        lambda = newLambda;
        break;
      }
      stepScale *= 0.5;
  ...
  // Final compute even if not converged
  const pi = computePi(lambda);
...
function calculateFlameTemperature(gasComponents, oxidizerType, excessOxygen, fuelTemperature, oxidizerTemperature) {
  ...
  const frozenEnthalpy = (T) => {
    ...
    return nCO2 * enthalpy('CO₂', T) + nH2O * enthalpy('H₂O', T) + nO2 * enthalpy('O₂', T) + nN2 * enthalpy('N₂', T);
  };
  ...
  return { theoretical: Math.max(0, TfrozenK - 273.15), actual: Math.max(0, TeqK - 273.15), stoichO2, composition, Hreact, b };
}
...
console.log('Corrected algorithm verification results:');
console.log('=========================================');
const results = {};
for (const c of cases) {
  ...
  if (r) {
    ...
    for (const sp in eq) {
      const moles = eq[sp]; const comp = atomicComp[sp];
      cBal += moles * comp.c; hBal += moles * comp.h; oBal += moles * comp.o; nBal += moles * comp.n; total += moles;
    }
...
fs.writeFileSync('frontend_corrected_results.json', JSON.stringify(results, null, 2));
