// Exact replica of FlameTemperaturePage.tsx calculateFlameTemperature logic
...
// Test cases matching cantera_verify.py
const testCases = [
...
console.log('FLAME TEMPERATURE VALIDATION: Web Logic vs Cantera');
console.log('='.repeat(80));
...
for (const tc of testCases) {
  ...
  console.log(`\nCase: ${tc.name}`);
  console.log(`  Fuel: ${tc.fuel.map(f=>f.symbol+'='+f.percentage).join(', ')} | Ox: ${tc.ox} | Excess: ${tc.excess}% | Tfuel: ${tc.fT}°C | Tox: ${tc.oT}°C`);
  ...
  // Compare major species (>1%)
  const webMajor = Object.entries(res.composition).filter(([k,v]) => v > 1).sort((a,b) => b[1]-a[1]);
  const canteraMajor = Object.entries(ref.products_mole_pct).filter(([k,v]) => v > 1).sort((a,b) => b[1]-a[1]);

  console.log(`  Major Species Comparison (>1%):`);
  const allMajor = new Set([...webMajor.map(x=>x[0]), ...canteraMajor.map(x=>x[0])]);
  for (const sp of Array.from(allMajor).sort()) {
    const w = res.composition[sp] || 0;
    const c = ref.products_mole_pct[sp] || 0;
    const diff = w - c;
    const pctDiff = c > 0 ? (diff/c*100).toFixed(1) : 'N/A';
