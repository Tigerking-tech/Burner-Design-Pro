...
console.log('=== Flame Temperature Validation Tests ===\n');

const testCases = [
  { name: 'CH₄ + Air (stoichiometric)', fuel: { 'CH₄': 100 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 0, expected: 1950 },
  { name: 'CH₄ + Air (10% excess)', fuel: { 'CH₄': 100 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 10, expected: 1870 },
  { name: 'CH₄ + Air (50% excess)', fuel: { 'CH₄': 100 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 50, expected: 1600 },
  { name: 'CH₄ + Pure O₂ (6% excess)', fuel: { 'CH₄': 100 }, fuelTemp: 0, oxidizer: 'oxygen', oxidizerTemp: 0, excessO2: 6, expected: 2780 },
  { name: 'H₂ + Air (stoichiometric)', fuel: { 'H₂': 100 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 0, expected: 2100 },
  { name: 'H₂ + Pure O₂ (6% excess)', fuel: { 'H₂': 100 }, fuelTemp: 0, oxidizer: 'oxygen', oxidizerTemp: 0, excessO2: 6, expected: 2800 },
  { name: 'CO + Air (stoichiometric)', fuel: { 'CO': 100 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 0, expected: 2400 },
  { name: 'C₃H₈ + Air (stoichiometric)', fuel: { 'C₃H₈': 100 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 0, expected: 1970 },
  { name: 'Natural Gas H + Air (10% excess)', fuel: { 'CH₄': 95, 'C₂H₆': 2.5, 'C₃H₈': 0.8, 'N₂': 1.2, 'CO₂': 0.5 }, fuelTemp: 25, oxidizer: 'air', oxidizerTemp: 25, excessO2: 10, expected: 1880 }
];

for (const test of testCases) {
  console.log(`\n--- ${test.name} ---`);
  console.log(`Fuel: ${JSON.stringify(test.fuel)}`);
  console.log(`Fuel Temp: ${test.fuelTemp}°C, Oxidizer: ${test.oxidizer}, Oxidizer Temp: ${test.oxidizerTemp}°C, Excess O₂: ${test.excessO2}%`);

  const result = calculateFlameTemperature(test.fuel, test.fuelTemp, test.oxidizer, test.oxidizerTemp, test.excessO2);

  if (result) {
    console.log(`Theoretical: ${result.theoretical.toFixed(1)}°C`);
    console.log(`Actual: ${result.actual.toFixed(1)}°C`);
    console.log(`Expected: ~${test.expected}°C`);
    console.log(`Error: ${Math.abs(result.actual - test.expected).toFixed(1)}°C (${((Math.abs(result.actual - test.expected) / test.expected) * 100).toFixed(2)}%)`);

    const comp = result.composition;
    console.log('\nCombustion Products (moles per mole fuel):');
    const sorted = Object.entries(comp).sort((a, b) => b[1] - a[1]);
    for (const [sp, moles] of sorted) {
      if (moles > 0.0001) {
