...
def init_db() -> None:
    ...
    try:
        ...
        try:
            cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_enabled INTEGER NOT NULL DEFAULT 0")
            conn.commit()
            print("[INFO] Added two_factor_enabled column")
        except Exception as e:
            print(f"[INFO] two_factor_enabled column may already exist: {e}")
            conn.rollback()

        try:
            cur.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_secret TEXT")
            conn.commit()
            print("[INFO] Added two_factor_secret column")
        except Exception as e:
        ...
        cur.execute("""
            CREATE INDEX IF NOT EXISTS idx_login_activities_user_id 
            ON login_activities(user_id)
        """)
        cur.execute("""
            CREATE INDEX IF NOT EXISTS idx_login_activities_created_at 
...
def delete_user(user_id: str) -> bool:
    ...
    try:
        ...
        # Delete related data first (orders, etc.)
...
def get_user_orders(user_id: str) -> List[Dict[str, Any]]:
    """Get all orders for a user (for data export)."""
    conn = _get_conn()
    try:
...
def save_login_activity(
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO login_activities (id, user_id, email, success, ip_address, user_agent, failure_reason, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            activity_id, user_id, email, 1 if success else 0,
            ip_address, user_agent, failure_reason, now
        ))
        conn.commit()
        cur.close()
    finally:
...
def get_user_login_activities(user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
